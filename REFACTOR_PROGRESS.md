...
**2026-04-26 发现**：ff 路径是手机端路由，不是死代码。`_isMobile=/iPhone|iPad|iPod|Android|Mobile/i.test(navigator.userAgent)` 在 send() 里检查着。改作抽出为 streamPollLoop（已完成）。

#### E. send() 拆 4 个小函数 ⭐ 推荐下一个
旧轮询路径在 line ~1340-1410（自带独立 `let el=null`）。前置 `grep -rn 'ffEnabled\|/api/chat/send\b\|/api/chat/history\b'` 确认无活跃调用方再删。**做完 send() 能掉 70+ 行。**
