...
http.createServer(async(req,res)=>{
  ...
  }else if(url.pathname.startsWith('/api/games/reviews')){
    // 游戏 Aha Moment 评测系统
    // 数据模型：{ id, meta:{name,icon,genre,...}, ratings:[{ratingId,user,score,aha,verdict,notes,ts}], aiSummary, createdAt, updatedAt }
    // 兼容旧扁平格式（自动 migrate on read）
    const GAMES_DIR=path.join(ROOT,'.openclaw','games','reviews');
    try{fs.mkdirSync(GAMES_DIR,{recursive:true});}catch{}
    const META_KEYS=['name','icon','genre','platform','monetization','target','sessionLen','ahaSummary','competitors','notes'];
    function migrate(d){
      if(d.meta&&Array.isArray(d.ratings))return d;
      const meta={};META_KEYS.forEach(k=>{if(d[k]!=null)meta[k]=d[k]});
      const ratings=[];
      // 老数据如果有 score/aha/verdict 当作一条匿名评分
      if(d.score!=null||d.aha||d.verdict){
        ratings.push({ratingId:'r0_legacy',user:d.author||'Anonymous',score:d.score!=null?Number(d.score):null,aha:d.aha||{},verdict:d.verdict||'',notes:d.notes||'',ts:d.createdAt||Date.now()});
      }
      return {id:d.id,meta,ratings,aiSummary:d.aiSummary||'',createdAt:d.createdAt||Date.now(),updatedAt:d.updatedAt||Date.now()};
    }
    function aggregate(rec){
      const rs=rec.ratings||[];
      if(!rs.length)return {count:0,score:null,verdict:'',ahaAvg:{},std:0,verdictCounts:{go:0,maybe:0,no:0}};
      const scored=rs.filter(r=>r.score!=null).map(r=>Number(r.score));
      const score=scored.length?(scored.reduce((a,b)=>a+b,0)/scored.length):null;
      const std=scored.length>1?Math.sqrt(scored.reduce((s,x)=>s+(x-score)**2,0)/scored.length):0;
      const ahaKeys=['firstTouch','firstWin','ahaMoment','loopHook','retention'];
      const ahaAvg={};
      ahaKeys.forEach(k=>{const vs=rs.map(r=>Number((r.aha||{})[k]||0)).filter(v=>!isNaN(v));ahaAvg[k]=vs.length?(vs.reduce((a,b)=>a+b,0)/vs.length):0});
      const vc={go:0,maybe:0,no:0};rs.forEach(r=>{if(vc[r.verdict]!=null)vc[r.verdict]++});
      const verdict=Object.keys(vc).reduce((a,b)=>vc[b]>vc[a]?b:a,'go');
      return {count:rs.length,score:score!=null?Math.round(score*10)/10:null,verdict:vc[verdict]?verdict:'',ahaAvg,std:Math.round(std*100)/100,verdictCounts:vc};
    }
    // 路由匹配：/api/games/reviews [GET|POST] / :id [GET|PUT|DELETE] / :id/ratings [POST] / :id/ratings/:rid [DELETE] / :id/summary [GET] / :id/ai-summary [POST]
    const mList=url.pathname.match(/^\/api\/games\/reviews\/?$/);
    const mItem=url.pathname.match(/^\/api\/games\/reviews\/([\w-]+)$/);
    const mRatings=url.pathname.match(/^\/api\/games\/reviews\/([\w-]+)\/ratings\/?$/);
    const mRating=url.pathname.match(/^\/api\/games\/reviews\/([\w-]+)\/ratings\/([\w-]+)$/);
    const mSummary=url.pathname.match(/^\/api\/games\/reviews\/([\w-]+)\/summary$/);
    const mAi=url.pathname.match(/^\/api\/games\/reviews\/([\w-]+)\/ai-summary$/);
    function loadRec(id){const fp=path.join(GAMES_DIR,id+'.json');if(!fs.existsSync(fp))return null;return migrate(JSON.parse(fs.readFileSync(fp,'utf-8')));}
    function saveRec(rec){rec.updatedAt=Date.now();fs.writeFileSync(path.join(GAMES_DIR,rec.id+'.json'),JSON.stringify(rec,null,2));}
    try{
      if(mList&&req.method==='GET'){
        const items=[];
        for(const f of fs.readdirSync(GAMES_DIR)){
          if(!f.endsWith('.json'))continue;
          try{const rec=migrate(JSON.parse(fs.readFileSync(path.join(GAMES_DIR,f),'utf-8')));
            const agg=aggregate(rec);
            items.push({id:rec.id,...rec.meta,score:agg.score,verdict:agg.verdict,reviewCount:agg.count,std:agg.std,updatedAt:rec.updatedAt});
          }catch{}
        }
        items.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
        sendJson(req,res,{items});
      }else if(mList&&req.method==='POST'){
        // 新建游戏（只设 meta，可选带一条首发评分）
        const body=await readBody(req);const d=JSON.parse(body);
        const meta={};META_KEYS.forEach(k=>{if(d[k]!=null)meta[k]=d[k]});
        const ratings=[];
        if(d.score!=null||d.aha||d.verdict){
          ratings.push({ratingId:'r'+Date.now().toString(36)+Math.random().toString(36).slice(2,6),user:(d.user||'Anonymous').slice(0,50),score:d.score!=null?Number(d.score):null,aha:d.aha||{},verdict:d.verdict||'',notes:d.notes||'',ts:Date.now()});
        }
        const rec={id:d.id||('g'+Date.now().toString(36)+Math.random().toString(36).slice(2,6)),meta,ratings,aiSummary:'',createdAt:Date.now(),updatedAt:Date.now()};
        saveRec(rec);
        sendJson(req,res,{ok:true,id:rec.id});
      }else if(mItem&&req.method==='GET'){
        const rec=loadRec(mItem[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        sendJson(req,res,{...rec,summary:aggregate(rec)});
      }else if(mItem&&req.method==='PUT'){
        // 更新元信息（不动 ratings）
        const body=await readBody(req);const d=JSON.parse(body);
        const rec=loadRec(mItem[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        META_KEYS.forEach(k=>{if(d[k]!=null)rec.meta[k]=d[k]});
        saveRec(rec);
        sendJson(req,res,{ok:true,id:rec.id});
      }else if(mItem&&req.method==='DELETE'){
        const fp=path.join(GAMES_DIR,mItem[1]+'.json');if(fs.existsSync(fp))fs.unlinkSync(fp);
        sendJson(req,res,{ok:true});
      }else if(mRatings&&req.method==='POST'){
        // 加一份评分
        const body=await readBody(req);const d=JSON.parse(body);
        const rec=loadRec(mRatings[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        const rating={ratingId:'r'+Date.now().toString(36)+Math.random().toString(36).slice(2,6),user:(d.user||'Anonymous').slice(0,50),score:d.score!=null?Number(d.score):null,aha:d.aha||{},verdict:d.verdict||'',notes:d.notes||'',ts:Date.now()};
        rec.ratings.push(rating);
        rec.aiSummary=''; // 数据变了，旧总结作废
        saveRec(rec);
        sendJson(req,res,{ok:true,ratingId:rating.ratingId});
      }else if(mRating&&req.method==='DELETE'){
        const rec=loadRec(mRating[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        rec.ratings=rec.ratings.filter(r=>r.ratingId!==mRating[2]);
        rec.aiSummary='';
        saveRec(rec);
        sendJson(req,res,{ok:true});
      }else if(mRating&&req.method==='PUT'){
        // 更新某条评分（场景：自己改自己的）
        const body=await readBody(req);const d=JSON.parse(body);
        const rec=loadRec(mRating[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        const idx=rec.ratings.findIndex(r=>r.ratingId===mRating[2]);
        if(idx<0){res.writeHead(404);res.end('{"error":"rating not found"}');return;}
        const r=rec.ratings[idx];
        if(d.user!=null)r.user=String(d.user).slice(0,50);
        if(d.score!=null)r.score=Number(d.score);
        if(d.aha)r.aha=d.aha;
        if(d.verdict!=null)r.verdict=d.verdict;
        if(d.notes!=null)r.notes=d.notes;
        r.ts=Date.now();
        rec.aiSummary='';
        saveRec(rec);
        sendJson(req,res,{ok:true});
      }else if(mSummary&&req.method==='GET'){
        const rec=loadRec(mSummary[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        sendJson(req,res,{summary:aggregate(rec),aiSummary:rec.aiSummary||'',ratings:rec.ratings,meta:rec.meta});
      }else if(mAi&&req.method==='POST'){
        // AI 立项总结：调用 OpenClaw gateway 跑一次 LLM
        const rec=loadRec(mAi[1]);if(!rec){res.writeHead(404);res.end('{"error":"not found"}');return;}
        const agg=aggregate(rec);
        const prompt='你是一位资深手游制作人，正在帮团队评估一款【'+(rec.meta.name||'未命名')+'】游戏（'+(rec.meta.genre||'')+'，目标用户：'+(rec.meta.target||'未指定')+'）是否值得立项。\n\n'+
          '## 已有 '+agg.count+' 份评测\n'+
          '- 综合评分：'+(agg.score==null?'未打分':agg.score+'/10')+(agg.std?'（分歧度 σ='+agg.std+'）':'')+'\n'+
          '- 立项投票：✅值得做 '+agg.verdictCounts.go+' 票 / ⚠️谨慎 '+agg.verdictCounts.maybe+' 票 / ❌别碰 '+agg.verdictCounts.no+' 票\n'+
          '- Aha 5 维度均分：First Touch '+agg.ahaAvg.firstTouch.toFixed(1)+' / First Win '+agg.ahaAvg.firstWin.toFixed(1)+' / Aha Moment '+agg.ahaAvg.ahaMoment.toFixed(1)+' / Loop Hook '+agg.ahaAvg.loopHook.toFixed(1)+' / Retention '+agg.ahaAvg.retention.toFixed(1)+'\n\n'+
          '## 各人评测明细\n'+rec.ratings.map(r=>'- '+r.user+': '+(r.score!=null?r.score+'/10':'未打分')+' '+(r.verdict==='go'?'✅':r.verdict==='maybe'?'⚠️':r.verdict==='no'?'❌':'')+(r.notes?' // '+r.notes:'')).join('\n')+'\n\n'+
          (rec.meta.ahaSummary?'## 核心爽点\n'+rec.meta.ahaSummary+'\n\n':'')+
          (rec.meta.competitors?'## 竞品对比\n'+rec.meta.competitors+'\n\n':'')+
          '请用 200 字以内输出立项判断，结构：\n1. 一句话结论（值得做 / 谨慎 / 不建议）\n2. 最大优势（1 条）\n3. 最大风险（1 条）\n4. 关键分歧点（如果 σ>1 或 verdict 不一致）\n5. 行动建议（要不要做、做的话从哪切入）';
        // 调 OpenClaw gateway
        let token='';try{const cfg=JSON.parse(fs.readFileSync(path.join(ROOT,'.openclaw','openclaw.json'),'utf-8'));token=cfg?.gateway?.auth?.token||'';}catch{}
        const http=require('http');
        const postBody=JSON.stringify({model:'openclaw/opus',messages:[{role:'user',content:prompt}],stream:false});
        const aiReq=http.request({host:'127.0.0.1',port:18789,path:'/v1/chat/completions',method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token,'Content-Length':Buffer.byteLength(postBody)}},aiRes=>{
          let buf='';aiRes.on('data',c=>buf+=c);aiRes.on('end',()=>{
            try{const j=JSON.parse(buf);const text=j?.choices?.[0]?.message?.content||'';rec.aiSummary=text;rec.aiSummaryAt=Date.now();saveRec(rec);sendJson(req,res,{ok:true,aiSummary:text});}
            catch(e){res.writeHead(502);res.end(JSON.stringify({error:'ai parse failed',detail:buf.slice(0,500)}));}
          });
        });
        aiReq.on('error',e=>{res.writeHead(502);res.end(JSON.stringify({error:e.message}))});
        aiReq.write(postBody);aiReq.end();
      }else{res.writeHead(405);res.end('{"error":"method not allowed"}');}
    }catch(e){res.writeHead(500);res.end(JSON.stringify({error:e.message}));}
